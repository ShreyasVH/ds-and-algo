...
public class Problem700233
{
	...
	public int[] getPostorder(int[] inorder, int[] preorder) {
        ...
        buildPostOrder(inorder, preorder, 0, inorder.length - 1, new int[]{0}, postorder, new int[]{0});

        return postorder;
    }

    public int searchIndex(int value, int[] inorder)
    {
        int index = -1;
    ...
    public void buildPostOrder(int[] inorder, int[] preorder, int inStart, int inEnd, int[] preIndex, int[] postorder, int[] postIndex) {
        if (inStart > inEnd) {
            return;
        }
        ...
        int rootIndex = searchIndex(root, inorder);

        buildPostOrder(inorder, preorder, inStart, rootIndex - 1, preIndex, postorder, postIndex);
        buildPostOrder(inorder, preorder, rootIndex + 1, inEnd, preIndex, postorder, postIndex);
...
Space complexity: O(h) [O(log n) for balanced tree]
                        [O(n) for skewed tree]
*/